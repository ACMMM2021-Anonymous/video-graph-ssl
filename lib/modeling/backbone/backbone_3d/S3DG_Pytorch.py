...
class S3DG(nn.Module):
    ...
    def __init__(self, num_classes=400, dropout_keep_prob = 1, input_channel = 3, spatial_squeeze=True, use_softmax=False):
...
if __name__=='__main__':
    ...
    if not os.path.exists('models/RGB_imagenet.pkl'):
        print('No weights Found! please download first, or comment 382~384th line')
    model.load_state_dict('models/RGB_imagenet.pkl')
    #model = model.cuda()
    data = torch.randn(2, 3, 64, 224, 224)#.cuda())
    out = model(data)
    print(out.size())
